...
private:
    ...
    if (show_window_) {
      ...
      if (elapsed >= 1.0) {
        ...
        const char* state_str = current_state_ == IDLE ? "IDLE" :
                                current_state_ == TRACKING ? "TRACKING" : "GRAB";
  ...
  void publish_packet(float distance) {
    using packet_t = gdut::data_packet<gdut::crc16_algorithm>;
    ...
    auto packet = std::make_shared<packet_t>(pole_code_, body.begin(), body.end(),
                                             gdut::build_packet);
    if (*packet) {
